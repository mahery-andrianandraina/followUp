...
(function initMailAlerts() {
    ...
    // ── Récupérer le PO depuis state.data au moment de l'envoi ─
    function extractPO(item) {
        if (!item.rowIndex || !item.sheet) return "";
        const rows = window.state?.data?.[item.sheet] || [];
        const row  = rows.find(r => r._rowIndex === item.rowIndex);
        if (!row) return "";
        return String(
            row["PO"]         ||
            row["PO #"]       ||
    ...
    function buildXLSXBase64(alerts) {
        return new Promise((resolve, reject) => {
            ...
            _waitForXLSX(function(XL) {
                ...
                const HEADERS = ["Date","Menu","Style","Client","PO / Réf","Urgence","Alerte","Action requise","Détail"];
                const ws1 = XL.utils.json_to_sheet(summaryRows, { header: HEADERS });
                ws1["!cols"] = [{wch:12},{wch:22},{wch:15},{wch:15},{wch:18},{wch:12},{wch:45},{wch:45},{wch:40}];
                ...
                Object.entries(alerts).forEach(([, data]) => {
                    ...
                    ws["!cols"] = [{wch:15},{wch:15},{wch:18},{wch:12},{wch:45},{wch:45},{wch:40}];
    ...
    function buildEmailHTML(alerts) {
        ...
        Object.entries(alerts).forEach(([, data]) => {
            ...
            // En-tête section menu
            tableBody += `
            <tr>
                <td colspan="6" style="padding:10px 14px 8px;
            ...
            data.items.forEach(item => {
                ...
                <tr style="border-bottom:1px solid #f3f4f6;">
                    ...
                    <td style="padding:8px 10px;font-size:12px;font-weight:600;
                        color:#1a5296;white-space:nowrap;">${po || "—"}</td>
            ...
            tableBody += `<tr><td colspan="6" style="padding:4px 0;"></td></tr>`;
        });
...
<table width="100%" cellpadding="0" cellspacing="0"
    style="max-width:820px;margin:24px auto;background:#ffffff;
           border-radius:14px;overflow:hidden;
           box-shadow:0 2px 16px rgba(0,0,0,0.08);">

    <!-- HEADER -->
    <tr>
        <td style="background:linear-gradient(135deg,#1565c0 0%,#1e88e5 100%);padding:22px 28px;">
            <table cellpadding="0" cellspacing="0"><tr>
                ...
                <td>
                    <div style="color:#fff;font-size:16px;font-weight:600;">Rapport d'alertes quotidien</div>
                    <div style="color:rgba(255,255,255,0.72);font-size:12px;margin-top:3px;">${todayCap}</div>
                </td>
            </tr></table>
        </td>
    </tr>
        ...
        <td style="padding:16px 28px;background:#f8fafc;border-bottom:1px solid #e5e7eb;">
            <table cellpadding="0" cellspacing="0"><tr>
                <td style="padding-right:10px;">
                    <div style="background:#fee2e2;border-radius:10px;padding:10px 16px;
                        text-align:center;min-width:72px;">
                        <div style="font-size:24px;font-weight:700;color:#991b1b;">${counts.high}</div>
                        <div style="font-size:10px;color:#b91c1c;text-transform:uppercase;
                            letter-spacing:.06em;margin-top:2px;">Urgentes</div>
                    </div>
                </td>
                <td style="padding-right:10px;">
                    <div style="background:#fef9c3;border-radius:10px;padding:10px 16px;
                        text-align:center;min-width:72px;">
                        <div style="font-size:24px;font-weight:700;color:#854d0e;">${counts.mid}</div>
                ...
                <td style="padding-right:10px;">
                    <div style="background:#eff6ff;border-radius:10px;padding:10px 16px;
                        text-align:center;min-width:72px;">
                        <div style="font-size:24px;font-weight:700;color:#1e40af;">${counts.low}</div>
                ...
                <td>
                    <div style="background:#f0fdf4;border-radius:10px;padding:10px 16px;
                        text-align:center;min-width:72px;border:1px solid #bbf7d0;">
                        <div style="font-size:24px;font-weight:700;color:#166534;">${counts.total}</div>
            ...
            <div class="modal-body" style="padding:1.25rem 1.5rem;display:flex;flex-direction:column;gap:16px;">
                ...
                <div>
                    <label class="form-label" style="margin-bottom:5px;">
                        Destinataire(s) <span style="color:var(--danger,#dc2626)">*</span>
                    </label>
