...
function renderStyleModalBody() {
    ...
    <table class="style-detail-table"><thead><tr><th>GMT Color</th><th>Pantone</th><th>Color Code</th><th>Approval</th><th>PO</th><th>Articles</th><th>Prepack Barcode</th><th style="width:80px;text-align:center;">Actions</th></tr></thead><tbody id="style-table-body">`;
    if (styleRows.length) {
        styleRows.forEach(r => { const _apvl = r["Approval"] || ""; const _apvlCls = _apvl === "Approved" ? "color:var(--success,#16a34a);font-weight:600" : _apvl === "Rejected" ? "color:var(--danger,#dc2626);font-weight:600" : "color:var(--text-muted,#94a3b8)"; html += `<tr data-row="${r._rowIndex}" id="style-row-${r._rowIndex}"><td><span class="color-badge">${esc(r["GMT Color"]) || "—"}</span></td><td>${esc(r["Pantone"]) || "—"}</td><td>${esc(r["Color Code"]) || "—"}</td><td style="${_apvlCls}">${_apvl || "—"}</td><td>${esc(r["PO"]) || "—"}</td><td>${esc(r["Articles"]) || "—"}</td><td>${esc(r["Prepack Barcode"]) || "—"}</td><td style="text-align:center;"><button class="btn btn-edit btn-icon btn-xs" onclick="editStyleRow(${r._rowIndex})"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button></td></tr>`; });
    } else { html += `<tr><td colspan="8" style="text-align:center;color:var(--text-muted);padding:1.5rem;">Aucune couleur enregistrée.</td></tr>`; }
...
function openGlobalNotifDrawer() {
    ...
    if (!document.getElementById("gnd-summary-styles")) {
        ...
        .gnd-badge-trims { background:#ECFDF5; color:#065F46; border:0.5px solid #6EE7B7; }
        `;
...
(function () {
    ...
    @keyframes fu-banner-in { from { opacity: 0; transform: translateY(-4px); } to { opacity: 1; transform: translateY(0); } }
    #fu-context-banner.warn { background: #fff8e1; color: #8a6000; border-top: 1px solid #ffe082; }
  ...
  `;
    ...
    <div id="fu-chatbot-panel">
      ...
      <div id="fu-token-bar-wrap">
        ...
        <div id="fu-token-bar-track"><div id="fu-token-bar"></div></div>
      </div>
      ...
      <div id="fu-suggestions">
        <div id="fu-sugg-header">
          <span id="fu-sugg-label">Suggestion</span>
          <div id="fu-sugg-dots">
            <div class="fu-dot-ind active" data-idx="0"></div>
            <div class="fu-dot-ind" data-idx="1"></div>
            <div class="fu-dot-ind" data-idx="2"></div>
          </div>
        </div>
        <div id="fu-slider-wrap">
          <div id="fu-slider-track">
            <div class="fu-slide">
              <button class="fu-suggestion">
                <div class="fu-sugg-icon">${SUGGESTIONS[0].icon}</div>
                <span class="fu-sugg-text">${SUGGESTIONS[0].text}</span>
                <span class="fu-sugg-arrow">↗</span>
              </button>
            </div>
            <div class="fu-slide">
              <button class="fu-suggestion">
                <div class="fu-sugg-icon">${SUGGESTIONS[1].icon}</div>
                <span class="fu-sugg-text">${SUGGESTIONS[1].text}</span>
                <span class="fu-sugg-arrow">↗</span>
              </button>
            </div>
            <div class="fu-slide">
              <button class="fu-suggestion">
                <div class="fu-sugg-icon">${SUGGESTIONS[2].icon}</div>
                <span class="fu-sugg-text">${SUGGESTIONS[2].text}</span>
                <span class="fu-sugg-arrow">↗</span>
              </button>
            </div>
          </div>
...
(function initCardToolbar() {
    ...
    if (!document.getElementById(TOOLBAR_STYLE_ID)) {
        ...
        .dbs-tb-btn.tb-tp.has-tp { color: var(--color-text-warning); }
        .dbs-tb-btn.tb-img.has-img { color: var(--color-text-success); }
...
function injectToolbar(card) {
    ...
    card.querySelectorAll(
        ...
        const btnTP = document.createElement("button");
        btnTP.className = "dbs-tb-btn tb-tp" + (tpUrl ? " has-tp" : "");
        btnTP.title = tpUrl ? "Voir / mettre à jour le Tech Pack" : "Ajouter un Tech Pack";
        btnTP.setAttribute("aria-label", tpUrl ? "Voir Tech Pack" : "Ajouter Tech Pack");
        btnTP.innerHTML = `<i class="ti ${tpUrl ? "ti-clipboard-check" : "ti-clipboard-text"}" aria-hidden="true"></i><span>TP</span>`;
        btnTP.onclick = (e) => {
            e.stopPropagation();
            if (typeof tpOpen === "function") tpOpen(styleCode, rowIdx ?? 2, tpUrl);
            else if (tpUrl) window.open(tpUrl, "_blank");
        };
