    ...
    tableBody.innerHTML = rows.map(row => {
        ...
        const cells = cfg.cols.map((c, i) => {
            ...
            if (c.key === "AWB" && state.activeSheet === "sample") {
                ...
                return `<td class="${sticky.trim()}" title="${esc(String(val))}">${esc(String(val)) || "<span style='color:var(--text-muted)'>—</span>"}</td>`;
            }
            if (c.key === "Approval") {
...
(function () {
  ...
  const API_URL = "function doPost(e) {
  const API_KEY = "csk-2t3nr3fee2hfjtwect2f6r9x99dt336xxv3pwptj5exhxmnj"; //
  ...
  try {
    ...
    // ── Détection AWB DHL ─────────────────────────────────────────
    const awbMatch = body.prompt.match(/\b([0-9]{10,12})\b/);
    if (awbMatch) {
      const trackingResult = trackDHL(awbMatch[1]);
      return ContentService
        .createTextOutput(JSON.stringify({
          candidates: [{ content: { parts: [{ text: trackingResult }] } }]
        }))
        .setMimeType(ContentService.MimeType.JSON);
    }

    const systemPrompt = `You are an intelligent assistant embedded in a textile/fashion production management system (AW27 Collection).
You analyze the data from the table currently displayed on the page and respond naturally and accurately, like an experienced product manager.

The system covers these modules (you detect automatically which one is active from the page title):
- Dashboard: global collection overview
- Details: style/article details and status
- Sample: sample tracking and approvals
- Ordering: PO and order tracking
- Fabric Devo: fabric/material development and approvals
- Bulk A4 & Shade Band: bulk color approval and shade band tracking
- Pre Shipment: pre-shipment quality control
- Marketing: marketing visuals and content status
- Fabric Analysis: fabric composition and analysis
- Trims Devo: trims and accessories development

Response rules:
- Always respond in French, naturally, like a human colleague would
- The data is in English — understand it and respond in French
- For style status: give exact status + key dates + pending actions + any comments
- For fabric/trims: mention ref, supplier, approval status, delays
- For orders/PO: mention PO number, supplier, quantity, delivery date, status
- Always quote exact references found in the data (style no, PO no, fabric ref, etc.)
- If multiple items match, list them clearly
- If something is late or blocked, flag it with ⚠️
- Never invent data — if info is missing, say so clearly
- If asked for a summary or analysis, highlight key stats and alerts first

Response format rules:
- Maximum 3-4 lines per answer
- No bullet points unless listing more than 3 items
- No introduction like "Based on the data..." or "According to the table..."
- Go straight to the answer
- If status: one line with ref + status + date
- If multiple items: short list, no extra commentary
- If nothing found: one sentence only

Module redirection rules:
- If the requested information is not found in the current page data, respond exactly like this (in French):
  "Je ne trouve pas cette information dans le tableau actuel.
  👉 Consultez le module **[nom du module]** pour cette donnée, ou cliquez sur **Actualiser** pour mettre à jour vos données."
- To identify the right module, use this mapping:
  * Sample info / approvals / comments → module Sample
  * PO / order / quantity / supplier / delivery → module Ordering
  * Fabric ref / material / composition / mill → module Fabric Devo ou Fabric Analysis
  * Trims / accessories / labels / buttons → module Trims Devo
  * Color / shade / bulk approval / shade band → module Bulk A4 & Shade Band
  * Pre-shipment / QC / inspection / defects → module Pre Shipment
  * Marketing / visuals / content / photos → module Marketing
  * Style overview / general info / description → module Details
- Never say "I don't know" — always redirect to the right module

Off-topic rules:
- If the user asks something unrelated to the collection or work (weather, jokes, general knowledge, personal questions, etc.), answer it briefly and naturally in French
- But always end with a light, friendly nudge like:
  "... 😄 Sur ce, revenons à nos affaires — une question sur la collection ?"
- Keep the off-topic answer very short (1-2 lines max), the redirect is the priority
- Never refuse to answer — just answer and redirect`;
    ...
    const response = UrlFetchApp.fetch("https://api.cerebras.ai/v1/chat/completions", {
      ...
      payload: JSON.stringify({
        model: "llama-3.3-70b",   // ← modèle Cerebras (même Llama 3.3 70B)
        messages: messages,
        max_tokens: 300,
        temperature: 0.2
      }),
    ...
    if (result.error) {
      return ContentService
        .createTextOutput(JSON.stringify({ error: result.error.message }))
        .setMimeType(ContentService.MimeType.JSON);
    }

    const text = result?.choices?.[0]?.message?.content || "Pas de réponse.";
...
// ── Tracking DHL ──────────────────────────────────────────────
function trackDHL(awb) {
  try {
    const url = "https://api-eu.dhl.com/track/shipments?trackingNumber=" + awb;
    ...
    if (!result.shipments || result.shipments.length === 0) {
      return "❌ AWB " + awb + " introuvable ou invalide.";
    }
    ...
    let summary = "📦 **AWB " + awb + "**\n";
    summary += "Statut : " + (status.description || "—") + "\n";
    summary += "Localisation : " + (status.location?.address?.addressLocality || "—") + "\n";
    summary += "Mise à jour : " + (status.timestamp?.slice(0, 10) || "—") + "\n";

    if (events.length > 0) {
      summary += "\nDerniers événements :\n";
      events.forEach(ev => {
        summary += "• " + (ev.timestamp?.slice(0, 10) || "—") + " — " + (ev.description || "—") + " (" + (ev.location?.address?.addressLocality || "—") + ")\n";
      });
    }
    ...
    return "❌ Erreur tracking DHL : " + err.message;
  }
}
